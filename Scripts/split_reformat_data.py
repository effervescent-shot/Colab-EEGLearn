# Utilities to split and reformat the data
...
def split_data(x, y, ratio, seed=1, keep_proportions=1):
    ...
    :param keep_proportions: keep the dream/no_dream distribution of the original
         dataset in the two subsets, by splitting separately each class. 
    """
...
def subsampling_labels(data, labels, shuffle = True, seed = 1):
    ...
    :shuffle: shuffle data before subsampling
    
    return:
    data : the subsampled data
    labels : the subsampled labels
...
def reformat_data_labels(data, labels, move_axis = True):
    ...
    return:
    data_reshape : the reshaped data
    labels_reshape : the reshaped labels
