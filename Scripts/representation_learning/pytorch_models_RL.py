...
def CNN_model(kernel,act_fn,n_channels):
        ######################### CNN #########################
        ######################### layer 1 of cnn #########################
...
class RL_2DConv(nn.Module):    
    def __init__(self, act_fn,kernel,d_layer,n_channels,dropoutRate,n_frames,time_pool_size,n_predicted_frames):
        """
        ...
        output:
		convnets_encoder: one layer before the representation layer
		convnets_decoder: the representation layer
        """
        
        super(RL_2DConv, self).__init__()
        ######################### CNN #########################                
        ### ---> integrate cnn layers <--- ####
        ...
        self.fc2 = nn.Linear(d_layer, 32*32*n_channels*n_predicted_frames) # representation learning, map to the original input dimension
        self.encoder = nn.Sequential(self.dropout,self.fc1)
    ...
    def forward(self, x): 
        ...
        convnets_encoder = self.encoder(convnets)
        convnets_decoder = self.FC(convnets)
        
        return convnets_encoder, convnets_decoder   



class RL_convpool_conv1d(nn.Module):        
    """
        video size: [n_samples,n_channels,n_frames,32,32]
        image size: [batch_size, n_channles, 32, 32]
    ...
    def __init__(self, act_fn, kernel, d_layer, n_channels, dropoutRate, n_frames, k_size_1d,n_predicted_frames):
        super(RL_convpool_conv1d, self).__init__()
        ######################### CNN #########################
        self.cnn, frame_size, H_neurons = CNN_model(kernel,act_fn,n_channels)          
        #after the stack of frames, the size of data becames: [n_samples, 128, 4, 4,n_frames]
        ...
        self.fc2 = nn.Linear(d_layer, 32*32*n_channels*n_predicted_frames)
        self.encoder = nn.Sequential(self.dropout,self.fc1)
        self.FC = nn.Sequential(self.dropout,self.fc1,self.dropout,self.fc2)
...
class RL_LSTM(nn.Module): 
    ...
    """   
    def __init__(self, act_fn,kernel,d_layer,n_channels,dropoutRate,n_frames,num_units,n_LSTM_layers,n_predicted_frames):               
    ...
    def forward(self, x): 
        ...
        # flatten and FC
        convnets_encoder = self.encoder(output)
        convnets_decoder = self.FC(output)           
        return convnets_encoder, convnets_decoder 



class RL_Mix(nn.Module):
    """
        video size: [n_samples,n_channels,n_frames,32,32]
        image size: [batch_size, n_channles, 32, 32]
    ...
    def __init__(self, act_fn,kernel,d_layer,n_channels,dropoutRate,n_frames,k_size_1d,num_units,n_LSTM_layers,n_predicted_frames):
        super(RL_Mix, self).__init__()
        ######################### CNN #########################
        self.cnn, frame_size, H_neurons = CNN_model(kernel,act_fn,n_channels)        
        #after the stack of frames, the size of data becames: [n_samples, 128, 4, 4,n_frames]  
    ...
    def forward(self, x): 
        ...
        # flatten and FC
        convnets_encoder = self.encoder(concat)
        convnets_decoder = self.FC(concat)           
        return convnets_encoder, convnets_decoder 
